...
- first_verification_gate: 배포 후 `curl -L https://virtue.oracle.shdkej.com` HTML에서 recent empty-state가 returning branch(`최근 덕행을 불러오는 중이에요.`)로 바뀌었는지, 또는 최근 기록 목록이 실제로 채워졌는지 확인한다.
