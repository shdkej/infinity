...
- updated_at_latest: 2026-06-15T0607Z
...
- 2026-06-15T0607Z **서브타입 소싱 마찰 스크린 완료 (cloud research).** Phone strap/tether 4종 서브타입 평가 결과: **Subtype A (손목+범용클립 스트랩) GO 판정** — 1688 MOQ 50–100 단가 700–2,500 KRW, 클레임 주원인(클립 강도·스트랩 길이)은 공급사 QA로 관리 가능, 옵션 2–3색으로 단순화 가능. 크로스바디 스트랩 WATCH (케이스 호환 해결 후 재평가). 패치+스트랩 HOLD. 보안 케이블 WATCH(번들 후보). Compression pouch: 롤업 WATCH 후순위 (대형 셀러 경쟁·내구성 클레임 부담), 세탁물 파우치 HOLD (수요 부족), 패킹큐브 WATCH(번들 후보). 추천: phone strap Subtype A 샘플 준비 단계 진입, compression pouch는 phone strap 검증 후 번들 후보로 재평가. 라이브 커머스/계정/공개 발행 액션 0. 아티팩트: `artifacts/naver-shopping-01/subtype-sourcing-friction-screen-2026-06-15.md`; 리포트: `reports/naver-shopping-01/2026-06-15T0607Z-research.html`.
