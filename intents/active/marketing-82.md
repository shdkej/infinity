...
- `marketing-83`: 홈 반환형 3표면 gating 규칙 제안 완료 (deed_saved ≥ 1 기준 분기)
...
## 클라우드 리서치 추가 발견 (2026-06-24T23:00Z)

archive 검토에서 Virtue 프론트엔드 소스 위치 후보 확인:
- `marketing-01` archive: `/home/ubuntu/dev/virtue-rebirth-app` — K8s 배포에 사용된 로컬 경로
- `marketing-77` archive: `apps/web/src/app/add/page.tsx` 수정 기록 → monorepo 구조 확인, `apps/web/` 하위에 Next.js 앱

**다음 로컬 검사 우선 순서**:
1. `/home/ubuntu/dev/virtue-rebirth-app/apps/web/src/app/page.tsx` — 홈(랜딩) 진입 파일
2. `/home/ubuntu/dev/virtue-rebirth-app/apps/web/` — `package.json` 확인, typecheck/lint 명령 파악
3. 랜딩페이지 관련 컴포넌트/레이아웃 파일 탐색

이 정보가 맞다면 다음 로컬 라우터 사이클에서 바로 소스 확인 → 구현 시작이 가능하다.
