...
- **후속 intent 연속성**: 같은 프로젝트의 후속 Inbox/Active/Waiting이 이미 있으면 재등록을 요구하지 말고 기존 식별자/phase 또는 명확한 새 번호 규칙으로 이어받아야 한다. 프로젝트성 작업은 설계·조사 intent가 Archive로 닫혀도 원래 목표가 남아 있으면 후속 intent를 자동 연결해야 하며, 사용자가 다시 요청해야만 이어지는 상태로 두지 않는다.
