...
- priority: high
- permission: L1
- mode: prepare 완료 → execute_local 대기
- goal: /add 경로를 단계별로 감사하여 J1-J4별 first value, 기존 이벤트, prelaunch 해석 금지선을 한 문서에 고정
- success_criteria: 문서에 input burden·output strength·click tax·deed_judged·deed_saved·J3 저장 전 정상 종료 명시; 코드/카피/계측 변경 0건
- context:
  - target_repo: virtue-rebirth-app
  - target_path: apps/web/docs/add-flow-input-output-balance-audit.md
  - source_note: /home/ubuntu/dev/knowledge-lab/source/external-links/marketing/2026-05-26-ai-onboarding-input-output.md
  - prior_audits: home-screen-fae-audit, three-screen-value-path-audit, activation-path-friction-audit, first-60-second-value-observation-script
- cloud_prepare: artifacts/marketing-21/add-flow-input-output-balance-audit.md (드래프트 완료)
- local_execution_prompt: artifacts/marketing-21/local-execution-prompt.md
- next_action: 로컬 Claude Code가 드래프트를 virtue-rebirth-app에 배치 → 검증 게이트 → L2 agent-approved push
