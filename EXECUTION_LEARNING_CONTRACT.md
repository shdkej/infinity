...
`max_minutes`는 안전 상한이며 예상시간이 아니다. 다음 Intent의 계획은 같은 유형의 완료 leaf 실측을 먼저 읽고, 다음 식으로 **예상 종료시간**을 제시한다.

`예상 종료 = 유사 leaf 실측 중앙값 합계 + publish/verify 고정비 + 검증된 rework 여유`

- 문서·근거 점검·Red 재검토처럼 실측 중앙값이 5분 이하인 leaf는 5분 단위로 묶어 같은 dispatcher cycle에서 실행·검증·원장 반영까지 닫는다.
- 30분은 분할 임계값일 뿐 기본 예산이 아니다. 30분 상한을 각 leaf의 예정시간으로 더해 긴 작업처럼 보이게 만들지 않는다.
- publish/verify 고정비(명시 파일 stage, commit/push, 원격 확인, terminal ledger)는 별도 한 항목으로 계획한다. leaf 완료 뒤 이 항목이 없으면 handoff를 재발행하지 않고 같은 cycle에서 종료한다.
- 유사 실측이 3개 미만이면 보수적 초기값을 쓰되, 작업 종료 시 반드시 실제값을 ledger에 추가한다. 3개 이상이면 EL-01의 중앙값을 다음 예측의 기본값으로 사용한다.
- 남는 마감 시간에는 계획에 미리 적은 독립 품질 leaf만 실행한다. 새로운 범위·반복 handoff·상태 점검으로 시간을 채우지 않는다.
