...
1. `intents/active/{id}.md` → `intents/archive/{id}.md`로 이동하고, archive 문서를 **canonical final index** 포맷으로 재작성한다.
   ...
   - 지식 판정 필드: `knowledge_status`, `knowledge_decision`, `knowledge_targets`, `knowledge_reflection`
...
9. Archive 직전에 `python3 scripts/check_knowledge_promotion.py {intent-id}`를 실행한다. 모든 완료 Intent는 다음 중 하나로 닫는다.
   - `knowledge_status: promoted` + `knowledge_decision: promote`: 반복 가능한 원칙·발견·판단 기준·실패 교훈을 agent-wiki의 실제 페이지에 반영하고 `knowledge_targets`에 경로를 기록한다.
   ...
   - `knowledge_status: superseded` + `knowledge_decision: supersede`: 기존 지식으로 대체된 경우 새 대상과 이유를 기록한다.
