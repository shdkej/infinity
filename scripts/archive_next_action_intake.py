...
ARCHIVE_DIR = ROOT.parent / "source" / "infinity" / "archive"
...
def has_similar_open_intent(open_text: str, source_id: str, next_action: str) -> str:
    ...
    for entry_id, block in entries:
        if re.search(rf"source_archive:\s*source/infinity/archive/{re.escape(source_id)}\.md", block):
            return entry_id
...
- target_agent: genie
- requested: {now}
- source_archive: source/infinity/archive/{source_id}.md
